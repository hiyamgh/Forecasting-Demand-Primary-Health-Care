...
from probabilistic_uncertainty.deep_ensemble_model import DeepEnsembleModel
import random
import os
...
def ensemble_mean_var(ensemble, xs, ys, sess):
    ...
    for model in ensemble:
        ...
        nll, mean, var = sess.run([model.loss, model.output_mu, model.output_sig_pos], feed)
        print('NLL in ensemble {}: {}'.format(i, nll))
        # _, nll, mean, var = sess.run([model.train_opt, model.loss, model.output_mu, model.output_sig_pos], feed)

        # sess.run([model.loss, model.output_mu, model.output_sig_pos])
    ...
    # en_var is actually the uncertainty in deep ensemble
