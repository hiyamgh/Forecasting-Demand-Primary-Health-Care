...
# t is a tuple of the following structure: (threshold - distance - method)
for t in combination1:
    print('************************************* {} *************************************'.format(t))
    vec = get_bool_method(t[2])
    lm = LearningModel(df=df_train, target_variable='demand', split_ratio=0.2,
                     output_folder='../output_ubr_shallow/%s_%s/%s_%s_%s/' % (KEY, c, t[2], t[0], t[1]),

                       # parameters for the relevance function phi
                     ...
                     input_zscore=None, input_minmax=(0,5), input_box=None, input_log=None,
