...
# services and mohafazas
services = ['General Medicine', 'Gynaecology', 'Pediatrics', 'Pharmacy']
mohafazas = ['akkar', 'bikaa', 'Tripoli']

output_folder = '../old_output/shallow_tsts/all_columns/train_separated_test_separated/'

# loop over files and check their service names and their mohafaza names
for file in glob.glob(datasets_separated[KEY] + '*.csv'):
    for service in services:
        for mohafaza in mohafazas:
            if service in file and mohafaza in file:
                # drop nan values and replace instances where demand is 0 with 1 to avoid MAPE being infinity
                df = pd.read_csv(file)
                df = df.dropna()
                ...
                lm = LearningModel(df, target_variable='demand',
                                    ...
                                    input_zscore=None, input_minmax=(0, 11) if 'w_{t-5}' in list(df.columns.values) else (0, 10), input_box=None, input_log=None,
                                    cols_drop=None,
